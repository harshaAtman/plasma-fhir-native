...
export function getAndCache(url: string, requestOptions?: RequestInit, force: boolean = process.env.NODE_ENV === "test"): Promise<any> {
    // ERM: Need to force it
    console.log("Cache", force, cache);
...
{
    // ERM: Pass in true as the 3rd parameter to force a reload of the conformance statement
    // The reason is because in the call to any(), it gets the .well-known and then aborts the conformance statement.
    // So the conformance statement is stored in cache as Aborted.
    // Not sure if I screwed this up or what.


    const url = String(baseUrl).replace(/\/*$/, "/") + "metadata";
    return getAndCache(url, requestOptions, true).catch((ex: Error) => {
        throw new Error(
